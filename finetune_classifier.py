...
tensorpath='./runs/%s/%s'%(task_name,localtime)
if not os.path.exists(tensorpath):
    os.mkdir(tensorpath)
writer = SummaryWriter(tensorpath)
...
def log_train(batch_id, batch_num, metric, step_loss, log_interval, epoch_id, learning_rate):
    ...
    writer.add_scalar('train_loss', step_loss / log_interval, epoch_id*lentrain+batch_id+1)
    writer.add_scalar('train_acc', metric_val[0], epoch_id*lentrain+batch_id+1)

    logging.info(train_str, epoch_id + 1, batch_id + 1, batch_num, step_loss / log_interval,
                 learning_rate, *metric_val)


def log_eval(batch_id, batch_num, metric, step_loss, log_interval,epoch_id):
    """Generate and print out the log message for inference. """
    metric_nm, metric_val = metric.get()
    if not isinstance(metric_nm, list):
        metric_nm, metric_val = [metric_nm], [metric_val]
    #writer.add_scalar('val_loss', step_loss / log_interval, epoch_id*lendev+batch_id+1)
    #writer.add_scalar('val_acc', metric_val[0], epoch_id*lendev+batch_id+1)
...
def train(metric):
    ...
    for epoch_id in range(epoch_number):
        ...
        if not only_inference:
            ...
            for batch_id, seqs in enumerate(train_data):
                ...
                if (batch_id + 1) % (args.log_interval) == 0:
                    #writer.add_scalar('train_loss', step_loss/args.log_interval, batch_id+lentrain*epoch_id)
        ...
        for segment, dev_data in dev_data_list:
            metric_nm, metric_val = evaluate(dev_data, metric, segment,epoch_id)
...
# def evaluate(loader_dev, metric, segment,train_epoch):
...
#         metric.update([label], [out])
#         if (batch_id + 1) % (args.log_interval) == 0:
#             #writer.add_scalar('dev_loss', step_loss/args.log_interval, batch_id+lentrain*train_epoch)
 
#             log_eval(batch_id, len(loader_dev), metric, step_loss, args.log_interval,train_epoch)
#             step_loss = 0

#     metric_nm, metric_val = metric.get()
#     print(metric_nm,metric_val)
#     #writer.add_scalar('val_loss', step_loss / log_interval，epoch_id)
#    # writer.add_scalar('val_acc', metric_val[0], epoch_id)
#     if not isinstance(metric_nm, list):
#         metric_nm, metric_val = [metric_nm], [metric_val]
...
def evaluate(loader_dev, metric, segment,train_epoch):
    ...
    writer.add_scalar('val_loss', all_loss / all_len,train_epoch)
    if not isinstance(metric_nm, list):
        metric_nm, metric_val = [metric_nm], [metric_val]
    metric_str = 'validation metrics:' + ','.join([i + ':%.4f' for i in metric_nm])
    logging.info(metric_str, *metric_val)
    for i in range(len(metric_nm)):
        writer.add_scalar('val_%s'%metric_nm[i], metric_val[i], train_epoch)
